...
def check_font(font='Arial.ttf', size=10):
    # Return a PIL TrueType Font, downloading to CONFIG_DIR if necessary
    font_path = Path(font)
    font_path = font_path if font_path.exists() else (CONFIG_DIR / font_path.name)
    try:
        return ImageFont.truetype(str(font_path) if font_path.exists() else font_path.name, size)
    except OSError as e:  # download if missing or if an unknown file format error occurs
        print(f"Error loading font '{font}': {e}. Attempting to download the font.")
        url = "https://ultralytics.com/assets/" + font_path.name
        print(f'Downloading {url} to {font_path}...')
        try:
            torch.hub.download_url_to_file(url, str(font_path), progress=False)
            return ImageFont.truetype(str(font_path), size)
        except Exception as download_exception:
            print(f"Failed to download the font from {url}. Error: {download_exception}")
